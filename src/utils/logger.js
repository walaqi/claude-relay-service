  ...
  module.exports = new Proxy(
    {
      ...
      start: consoleLogger('start'),
      request: (method, url, status, duration) =>
        console.log(`[REQUEST] ${method} ${url} - ${status} (${duration}ms)`),
      api: consoleLogger('api'),
      security: consoleLogger('security'),
      database: consoleLogger('database'),
      performance: consoleLogger('performance'),
      audit: consoleLogger('audit'),
      authDetail: consoleLogger('auth-detail'),
      timer: () => ({ end: () => 0 }),
      getStats: () => ({ requests: 0, errors: 0, warnings: 0 }),
      resetStats: () => {},
      healthCheck: () => ({ healthy: true, timestamp: new Date().toISOString() }),
      stats: { requests: 0, errors: 0, warnings: 0 }
    },
    handler
  )
} else {
