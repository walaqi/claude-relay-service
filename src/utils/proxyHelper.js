...
if (isWorkerMode()) {
  // Workers 模式下代理不可用（Agent API 为 no-op）
  class ProxyHelperStub {
    static _agentCache = new Map()
    static _warned = false

    static createProxyAgent() {
      if (!ProxyHelperStub._warned) {
        console.log(
          '[WARN] Proxy is not available in Workers mode — requests go directly from Cloudflare edge'
        )
        ProxyHelperStub._warned = true
      }
      return null
    }

    static createProxy() {
      return ProxyHelperStub.createProxyAgent()
    }
    ...
    static getProxyDescription(proxyConfig) {
      ...
      try {
        const proxy = typeof proxyConfig === 'string' ? JSON.parse(proxyConfig) : proxyConfig
        return `${proxy.type}://${proxy.host}:${proxy.port} (Workers: disabled)`
      } catch (_e) {
    ...
    static maskProxyInfo(proxyConfig) {
      return ProxyHelperStub.getProxyDescription(proxyConfig)
    }
  }

  module.exports = ProxyHelperStub
} else {
